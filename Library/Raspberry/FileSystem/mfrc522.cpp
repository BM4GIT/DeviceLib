...
String toHex( const uint8_t *data, const uint8_t size)
{
	String str;
	uint32_t szPos;
...
{
  m_spi = SPISettings( 1000000, MSBFIRST, SPI_MODE0);
}
...
bool Rfid::init()
{
  ...
  PCD_Init(); // without control mechanism for success or failure
  return true;
}

void Rfid::setTimeout( uint32_t msec)
{
...
void Rfid::read()
{
  ...
  if ( PICC_IsNewCardPresent() && PICC_ReadCardSerial() ) {
    m_tag = toHex( uid.uidByte, 4);
    m_stamp = millis();
    Sensor::read();
  }
}

String Rfid::tag( uint32_t expireAfter)
{
	if ( expireAfter > 0 && m_stamp + expireAfter > (uint32_t) millis() )
    return EXPIRED;
  return m_tag;
}
...
{
    for ( uint8_t i = 0; i < sizeof( RFIDKEY); i++ )
        m_key.keyByte[i] = key[i];
}

bool Rfid::readValue( uint16_t index, String& value)
{
    uint8_t bsize = 18;
    uint8_t buffer[bsize] = "                xx";
    uint8_t ixkey = 3;
    ...
    if ( PCD_Authenticate( PICC_CMD_MF_AUTH_KEY_A, ixkey, &m_key, &uid) != STATUS_OK )
      return false;
...
bool Rfid::writeValue( uint16_t index, String value)
{
    uint8_t bsize = 16;
    ...
    if ( PCD_Authenticate( PICC_CMD_MF_AUTH_KEY_A, ixkey, &m_key, &uid) != STATUS_OK )
      return false;
    if ( MIFARE_Write( index, buffer, bsize) != STATUS_OK )
      return false;
...
{
  union { uint16_t val; struct { uint8_t lsb; uint8_t msb; }; } data;
  data.lsb = reg & 0x7E;
  data.msb = value;
...
{
  union { uint16_t val; struct { uint8_t lsb; uint8_t msb; }; } data;
  data.lsb = 0x80 | (reg & 0x7E);
  data.msb = 1;
  data.val = SPI.transfer16( data.val);
  return (byte) data.msb;
}
...
void Rfid::PCD_Init()
{
  if ( digitalRead( m_rst) == LOW ) {	//The MFRC522 chip is in power down mode.
    digitalWrite( m_rst, HIGH);		// Exit power down mode. This triggers a hard reset.
