...
String toHex( const uint8_t *data, const uint size)
{
	String str;
	uint32_t szPos;
...
void Rfid::setPin( uint8_t pinIrq, uint8_t pinRst)
{
    begin( pinIrq, pinRst);
}
...
void Rfid::setTimeout( uint32_t timeout)
{
    m_timeout = timeout;
}
...
void Rfid::read()
{
    ...
    if ( readPassiveTargetID( PN532_MIFARE_ISO14443A, m_uid, &len, m_timeout) ) {
        m_tag = toHex( m_uid, sizeof( m_uid));
        m_stamp = millis();
        Sensor::read();
    }
}
...
bool Rfid::readdata( uint8_t* buff, uint8_t n)
{
  ...
  uint8_t buf[n+2];
  memset( buf, 0, n+2);
    Wire.requestFrom( PN532_I2C_ADDRESS, n+2);
    int i = 0;
    while ( Wire.available() && (i < n+2) ) {
        buf[i] = Wire.read();
        i++;
    }
...
bool Rfid::writecommand(uint8_t* cmd, uint8_t cmdlen)
{
...
uint32_t Rfid::getFirmwareVersion()
{
  ...
  int offset = 7;  // Skip a response uint8_t when using I2C to ignore extra data.
...
bool Rfid::SAMConfig()
{
  ...
  // read data packet
  if ( !readdata( pn532_packetbuffer, 8) ||
        (pn532_packetbuffer[6] != 0x15) ) {
    return false;
  }
...
bool Rfid::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint32_t timeout) 
{
  ...
  // read data packet
  pn532_packetbuffer[7] = 0; // clear uint8_t to be able to check on it later
