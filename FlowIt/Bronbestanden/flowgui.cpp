...
FlowGui::FlowGui( void* column) : FlowObject( column)
{
    m_fot = FOT_GUI;
}

void FlowGui::init( QString name, QString instance, Template* templ)
{
    m_name = name;
...
void FlowGui::read( QTextStream& in)
{
    QString ln, tag, val;
    QStringList func;
    m_templ = NULL;
    ...
    while ( !in.atEnd() ) {
        ...
        if ( tag == "</flowgui>" ) break;
    }
}

void FlowGui::write( QTextStream& out)
{
...
QString FlowGui::getInstance()
{
    return m_inst;
}

Template* FlowGui::getTemplate()
{
    return m_templ;
}

bool FlowGui::hasInstance( QString name)
{
...
void FlowGui::drawChart( QPainter& p)
{
    int w = width();
    int h = height();
    int x = m_xpos - w/2;
