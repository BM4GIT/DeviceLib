...
FlowStorage::FlowStorage( void* column) :
     FlowObject( column)
{
    m_fot = FOT_STORAGE;
    m_templ = NULL;
...
void FlowStorage::init( QString name, QString instance, Template* templ)
{
    m_name = name;
...
void FlowStorage::read( QTextStream& in)
{
    QString ln, tag, val;
    QStringList func;
    m_templ = NULL;
    while ( !in.atEnd() ) {
        ...
        if ( tag == "</flowstorage>" ) break;
    }

    if ( m_templ ) m_templ->init( m_fot, func);
}

void FlowStorage::write( QTextStream& out)
{
    ...
    out << g_tabs + "</flowstorage>\n";
}

QString FlowStorage::getInstance()
{
    return m_inst;
}

Template* FlowStorage::getTemplate()
{
    return m_templ;
}

bool FlowStorage::hasInstance( QString name)
{
    if ( m_inst == name ) return true;
    return FlowObject::hasInstance( name);
}

int FlowStorage::width()
{
    int w, h;
...
void FlowStorage::drawChart( QPainter& p)
{
    int w = width();
    int h = height();
    int x = m_xpos - w/2;
    ...
    p.drawArc( x + w - h, m_ypos + 1, h, h, 16*90, 16*180);
    ...
    p.drawText( QRect( x + 15, m_ypos, w - 25, h), Qt::AlignLeft, m_name);
    w -= 20;
    p.setPen( Qt::blue);
